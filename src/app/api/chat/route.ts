...
export async function POST(req: NextRequest) {
  try {
    ...
    // 상담 완료 Drive 업데이트만 요청한 경우 (AI 응답 없이 Drive만 저장)
    ...
    const systemWithContext =
      SYSTEM_PROMPT + `\n\n지금 이야기하는 학생 이름: ${studentName}\n상담 주제: ${roomTopic}`;

    const response = await client.messages.create({
      model: 'claude-sonnet-4-6',
      max_tokens: 600,
      system: systemWithContext,
      messages: messages.length === 0 ? [{ role: 'user', content: '안녕하세요' }] : messages,
    });

    const content = response.content[0].type === 'text' ? response.content[0].text : '';

    // AI 응답까지 포함한 전체 대화를 Google Drive에 저장 (비동기, 응답을 막지 않음)
    if (roomId) {
